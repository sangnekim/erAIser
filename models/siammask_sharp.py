...
def select_mask_logistic_loss(p_m, mask, weight, o_sz=63, g_sz=127):
    ...
    """
    unfold할 때, base model에서는 padding=32 사용 -> 이렇게 하면 차원 문제 발생 x
    근데 여기서(sharp)에서는 padding=0 사용 -> 이렇게 하면 차원 문제 발생
    아마 refine하는 과정에서 뭔가 차원 변화가 생겨서 padding=0으로 해둔 것 같아서 일단은 수정안했음 (5/21)
    나중에 train할 때 loss가 계산 안되는 문제가 생기면 그때 수정 필요할 듯
    함수들은 pytorch 1.8.1에서 문제없이 실행되는 것으로 확인! (5/21)
    """
...
if __name__ == "__main__":
    ...
    loss = select_mask_logistic_loss(p_m, mask, cls)  # 위에서 말한 계산 안된다는 부분 실행 코드
    print(loss)